        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mt-12">
          {[1, 2, 3, 4].map((i) => (
            <div key={i} className="relative group aspect-[4/3]">
              <div className={`absolute inset-0 ${i % 2 === 0 ? 'bg-[#0b1120]' : 'bg-[#10b981]'} rounded-3xl translate-y-2 translate-x-2 border-2 border-[#0b1120]`}></div>
              <div className="relative w-full h-full bg-white border-[3px] border-[#0b1120] rounded-3xl overflow-hidden">
                <img src={`https://picsum.photos/seed/meetup${i}/400/300`} alt="Community Meetup" className="w-full h-full object-cover" />
              </div>
            </div>
          ))}
        </div>
