...
function DocCard(props: { doc: DocEntry; key?: string }) {
  ...
  return (
    <Link
      to={`/docs/${doc.slug}`}
      className="group block rounded-2xl border-[3px] border-[#0b1120] p-6 bg-white shadow-[4px_4px_0px_#0b1120] hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[8px_8px_0px_#0b1120] transition-all"
    >
      <div className="text-4xl mb-4">{iconMap[doc.icon] || <BookOpen className="w-8 h-8 text-[#0b1120]" />}</div>
    ...
    <div className="min-h-screen bg-white text-[#0b1120] font-sans selection:bg-blue-100">
      <div className="max-w-7xl mx-auto px-6 py-24">
        ...
        <div className="text-center mb-16">
          <h1 className="text-5xl lg:text-6xl font-black text-[#0b1120] mb-6">Documentation</h1>
        ...
        <div className="max-w-xl mx-auto mb-16">
          <div className="relative">
            <svg className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input
              type="text"
              value={search}
              onChange={e => setSearch(e.target.value)}
              placeholder="Search documentation..."
              className="w-full rounded-2xl border-[3px] border-[#0b1120] bg-white pl-14 pr-6 py-4 font-bold text-[#0b1120] text-base placeholder:text-gray-400 placeholder:font-medium focus:outline-none focus:ring-2 focus:ring-[#10b981] shadow-[4px_4px_0px_#0b1120]"
            />
          </div>
        </div>
        ...
        {degreeDocs.length > 0 && (
          <div className="mb-16">
            <h2 className="text-2xl font-black text-[#0b1120] mb-6 flex items-center gap-3">
              <GraduationCap className="w-7 h-7" /> Degree
            </h2>
        ...
        {programmingDocs.length > 0 && (
          <div className="mb-16">
            <h2 className="text-2xl font-black text-[#0b1120] mb-6 flex items-center gap-3">
              <Code className="w-7 h-7" /> Programming Languages & References
            </h2>
